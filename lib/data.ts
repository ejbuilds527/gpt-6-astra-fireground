...
export type Snapshot = { settings: Record<string, Document>; site: Document; sources: Document; relay: Document; shuttle: Document; scenario: Document; scenarioId: string };
...
export async function readSnapshot(scenarioId = 'general-alarm'): Promise<Snapshot> {
  ...
  const [settings, docs] = await Promise.all([readSettings(), db.getAll(...paths.map(p => db.doc(p)))]);
  ...
  return { settings, site, sources, relay, shuttle, scenario, scenarioId };
}
