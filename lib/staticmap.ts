...
export const MAP_COLORS = {
  ...
  water: '#27343b', label: '#8f9aa4', hose: '#EF8200', loaded: '#5aabb3', ink: '#e6ebef',
} as const;
...
export function staticMapUrl({ apiKey, scale = 2, ...viewport }: StaticMapOptions): string {
  ...
  if (scale !== 1 && scale !== 2) throw new Error('Invalid map scale');
  ...
  for (const style of [
    ...
    'feature:poi|visibility:off', 'element:labels.icon|visibility:off', 'feature:transit|visibility:off',
