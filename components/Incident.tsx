...
export function Incident({role,identity,initial}:{role:Role;identity:RecordData;initial:RecordData|null}){
 ...
 return <main className="fg-console">
  ...
  {!data?<section><h2>Incident inputs unavailable</h2><p>Live connection is retrying. No supply values have been assumed.</p></section>:<>
   ...
   {role==='staging'&&<><section><p className="fg-eyebrow">WHERE TO STAGE</p><h2>{data.staging.point?.name||'POINT NOT SET'}</h2>{data.staging.point&&<p className="fg-caution">PROPOSED · BLOCKS LANE · parking lot not surveyed</p>}<p>{data.staging.point_reason}</p>{data.staging.overlap&&<p>Inbound overlap {data.staging.overlap.mutual_aid_to_staging_pct}% · final approach overlap {data.staging.overlap.staging_to_scene_pct}% · precomputed route-point measurements</p>}<h2 className="fg-caution">{data.hold_south?'HOLD — DO NOT ENTER THE FIRE ROAD UNTIL COMMAND CALLS YOU IN':'Hold released by command — confirm your assignment'}</h2><p>My state: <b>{data.states[identity.station]||'responding'}</b></p></section><section><h2>Who shares the southern approach</h2><ul>{(data.staging.split.south||[]).map((name:string)=><li key={name}>{name}</li>)}</ul><p>{data.staging.principle}</p></section></>}
